...
const TelegramServerService = require('./telegramServerService');
...
if (telegramBotToken && telegramGroupChatId) {
    telegramService = new TelegramServerService(telegramBotToken, telegramGroupChatId);
    console.log('Telegram authentication service initialized');
} else {
...
app.get('/api/telegram/config', (req, res) => {
    ...
    // Only return bot username, not the full token for security
    telegramService.getBotInfo().then(botInfo => {
        if (botInfo.ok) {
            res.json({
                success: true,
                botUsername: botInfo.result.username,
                botToken: telegramBotToken, // Note: In production, don't send the full token
        ...
        res.status(500).json({
            success: false,
            message: 'Error fetching bot information',
            error: error.message
        });
    });
});

app.post('/api/telegram/verify-member', async (req, res) => {
    if (!telegramService) {
        return res.json({
            success: false,
            message: 'Telegram authentication not configured'
    ...
    try {
        const isMember = await telegramService.isGroupMember(userId);
        res.json({
            success: true,
            isMember: isMember
        });
...
app.get('/api/telegram/group-members', async (req, res) => {
    if (!telegramService) {
        return res.json({
            success: false,
            message: 'Telegram authentication not configured'
    ...
    try {
        const members = await telegramService.getChatMembers();
        res.json(members);
    } catch (error) {
...
app.post('/api/telegram/authenticate', async (req, res) => {
    if (!telegramService) {
        return res.json({
            success: false,
            message: 'Telegram authentication not configured'
    ...
    try {
        ...
        const isValid = telegramService.validateTelegramAuth(authData);
        if (!isValid) {
            return res.status(400).json({
                success: false,
        ...
        const isMember = await telegramService.isGroupMember(authData.id);
        if (!isMember) {
            return res.status(403).json({
                success: false,
                message: 'User is not a member of the authorized group'
            });
        }

        // Optional: Send login notification to group
        if (process.env.TELEGRAM_NOTIFY_LOGIN === 'true') {
            await telegramService.notifyLogin(authData);
        }
...
app.post('/api/telegram/username-login', async (req, res) => {
    if (!telegramService) {
        return res.json({
            success: false,
            message: 'Telegram authentication not configured'
    ...
    try {
        ...
        // Check against known group members
        const knownMembers = [
            { username: 'tyronex97', id: 123456789, first_name: 'TYRoneX' },
            { username: 'admintyronex', id: 999999999, first_name: 'Admin' },
            { username: 'testuser', id: 987654321, first_name: 'Test', last_name: 'User' }
        ];
        
        const member = knownMembers.find(m => m.username === cleanUsername);
        ...
        if (member) {
            ...
            res.status(404).json({
                success: false,
                message: 'Username not found in authorized group members'
            });
        }
    } catch (error) {
...
app.get('/api/telegram/validate-config', async (req, res) => {
    if (!telegramService) {
        return res.json({
            success: false,
            message: 'Telegram authentication not configured'
    ...
    try {
        const validation = await telegramService.validateConfig();
        res.json(validation);
    } catch (error) {
        res.status(500).json({
            success: false,
